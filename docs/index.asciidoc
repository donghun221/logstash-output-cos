...
Requirements:
* Tencent COS Bucket and S3 Access Permissions (Typically access_key_id and secret_access_key)
...
output {
   cos{
     ...
     region => "eu-west-1"                    (optional, default = "guangzhou")
...
==== COS Output Configuration Options
...
| <<plugins-{type}s-{plugin}-region>> |<<string,string>>, one of `["ap-beijing-1", "ap-beijing", "ap-shanghai", "ap-guangzhou", "ap-chengdu", "ap-singapore", "ap-hongkong", "na-toronto", "eu-frankfurt"]`|No
...
| <<plugins-{type}s-{plugin}-storage_class>> |<<string,string>>, one of `["STANDARD", "STANDARD_IA"]`|No
...
3. Environment variables `COS_ACCESS_KEY_ID` and `COS_SECRET_ACCESS_KEY`
4. Environment variables `TENCENT_CLOUD_ACCESS_KEY_ID` and `TENCENT_CLOUD_SECRET_ACCESS_KEY`
5. IAM Instance Profile (available when running inside CVM)
...
===== `region` 
  ...
  * Value can be any of: `ap-beijing-1`, `ap-beijing`, `ap-shanghai`, `ap-guangzhou`, `ap-chengdu`, `ap-singapore`, `ap-hongkong`, `na-toronto`, `eu-frankfurt`
  * Default value is `"ap-guangzhou"`
...
The Tencent Cloud Secret Access Key
...
===== `server_side_encryption_algorithm` 
  ...
  * Value can be any of: `AES256`, `tencent:kms`
...
The key to use when specified along with server_side_encryption => tencent:kms.
...
===== `storage_class` 
  ...
  * Value can be any of: `STANDARD`, `STANDARD_IA`
